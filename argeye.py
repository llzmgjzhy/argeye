# Author:   <steven.bethard@gmail.com>.
...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def create_yml(self):
        # create argeye.yml if there is not,which concludes all the args and there default values

        # define yml file name and confirm if the work dir contains the file
        yml_name = "argeye.yml"
        current_dir = _os.getcwd()

        if _os.path.isfile(_os.path.join(current_dir, yml_name)):
            pass
        else:
            yml_data = {}
            for action in self._actions:
                if action.dest == "help":
                    continue
                if action.option_strings:
                    yml_data[min(action.option_strings, key=len)] = action.default
                else:
                    yml_data[action.dest] = action.default
            try:
                with open(yml_name, "w") as f:
                    _yaml.dump(yml_data, f, allow_unicode=True, sort_keys=False)
            except OSError as err:
                self.error(str(err))

    def read_yml(self):
        # read arguments from yml file,if exists,fetch args directly instead of input by command line.this way provides a more intuitive method to edit args
        yml_name = "argeye.yml"
        current_dir = _os.getcwd()
        yml_data = None
        ...
        if _os.path.isfile(_os.path.join(current_dir, yml_name)):
            try:
                with open(yml_name, "r") as f:
                    yml_data = _yaml.load(f.read(), Loader=_yaml.FullLoader)
                    yml_data = [
                        f"{key}={str(value)}" if key.startswith("-") else str(value)
    ...
    def parse_known_args(self, args=None, namespace=None):
        if args is None:
            ...
            args = _sys.argv[1:] if _sys.argv[1:] else self.read_yml()
        else:
